...
constexpr char to_upper(char c) { return c < 'a' || c > 'z' ? c : c - ('a' - 'A'); }
...
void test_cx_array() {
...

  {
    ...
    static_assert(m.size() == 6 && m[0] == 1 && m[1] == 9 && m[2] == 2, "array insert");
    constexpr auto n = test.insert<0>(9);
    static_assert(n.size() == 6 && n[0] == 9 && n[1] == 1, "array insert(0)");
    constexpr auto o = test.insert<5>(9);
    static_assert(o.size() == 6 && o[4] == 5 && o[5] == 9, "array insert(N)");
  }

  {
    constexpr auto p = cx::make_array("jackdwslovemybigphnxfqurtz");
    constexpr auto q = cx::sort(p, cless);
    static_assert(q[0] == 'a' && q[1] == 'b' && q[2] == 'c' && q[3] == 'd', "array sort");
  }

  {
    constexpr auto r = cx::make_array("Mad Hatter", "Alice", "March Hare", "Dormouse");
    constexpr auto s = cx::sort(r, cx::strless);
    static_assert(!cx::strcmp(s[0], "Alice") && !cx::strcmp(s[1], "Dormouse")
                      && !cx::strcmp(s[2], "Mad Hatter") && !cx::strcmp(s[3], "March Hare"),
                  "tea party");
  }
