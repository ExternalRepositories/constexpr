...
namespace cx {
  ...
  namespace detail {
    namespace murmur {
      ...
      constexpr uint32_t murmur3_32_loop(const char* key, int len, uint32_t hash) {
        return len == 0 ? hash
                        : murmur3_32_loop(key + 4, len - 1,
                                          murmur3_32_hashround(murmur3_32_k(word32le(key)), hash));
      }
      ...
      constexpr uint32_t murmur3_32_final3(uint32_t hash) { return (hash ^ (hash >> 16)); }
      ...
      constexpr uint32_t murmur3_32_value(const char* key, int len, uint32_t seed) {
        return murmur3_32_final(
            murmur3_32_end(murmur3_32_loop(key, len / 4, seed), key + (len / 4) * 4, len & 3), len);
      }
    }  // namespace murmur
  }    // namespace detail
