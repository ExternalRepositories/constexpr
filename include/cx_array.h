...
namespace cx {
  ...
  public:
    ...
    template <typename... Es, typename = typename std::enable_if<sizeof...(Es) <= N>::type>
    ...
    template <size_t... Is> constexpr array(const T* p, std::index_sequence<Is...>)
        : m_data{p[Is]...} {}
    ...
    template <typename F> constexpr auto map(F&& f) const -> array<decltype(f(T{})), N> {
    ...
    // array comparison
    template <size_t M> constexpr bool less(const array<T, M>& rhs) const {
    ...
    template <size_t M> constexpr array<T, (M + N)> concat(const array<T, M>& a) const {
      return concat(a, std::make_index_sequence<N>(), std::make_index_sequence<M>());
    }

    template <size_t A, size_t B, typename = typename std::enable_if<(A < B)>::type,
    ...
    template <size_t M = 1, typename = typename std::enable_if<(M < N)>::type>
    ...
    template <size_t I, typename = void> struct inserter;

    template <size_t I> constexpr array<T, N + 1> insert(const T& t) const {
      return inserter<I>()(*this, t);
    }

    // mergesort
    template <size_t I, typename = void> struct sorter;
    template <size_t I, size_t J, typename = void> struct merger;

    template <typename F> constexpr array<T, N> mergesort(F&& f) const {
      return sorter<N>::sort(*this, std::forward<F>(f));
    }

    template <typename P> constexpr array<T, N> partition(P&& p) const {
  ...
  private:
    ...
    template <typename F, size_t... Is> constexpr auto map(F&& f, std::index_sequence<Is...>) const
    ...
    constexpr auto map(F&& f, const array<U, M>& rhs, std::index_sequence<Is...>) const
      ...
      return array<decltype(f(T{}, U{})), sizeof...(Is)>{f(m_data[Is], rhs.m_data[Is])...};
    }
    ...
    constexpr array<T, (sizeof...(Is) + sizeof...(Js))> concat(const array<T, sizeof...(Js)>& a,
    ...
    template <size_t I> struct inserter<I, typename std::enable_if<(I > 0 && I < N)>::type> {
      constexpr array<T, N + 1> operator()(const array<T, N>& a, const T& t) const {
        return a.slice<0, I>().concat(a.slice<I, N>().push_front(t));
      }
    };

    template <size_t I> struct inserter<I, typename std::enable_if<(I == N)>::type> {
      constexpr array<T, N + 1> operator()(const array<T, N>& a, const T& t) const {
    ...
    template <size_t I> struct sorter<I, typename std::enable_if<(I == 1)>::type> {
      template <typename F> constexpr static array<T, I> sort(const array<T, I>& a, F&&) {
        return a;
      }
    };
    ...
    template <size_t I> struct sorter<I, typename std::enable_if<(I > 1)>::type> {
      template <typename F> constexpr static array<T, I> sort(const array<T, I>& a, const F& f) {
        return merger<I / 2, I - I / 2>::merge(
            a.init(std::make_index_sequence<I / 2>()).mergesort(f),
            a.tail(std::make_index_sequence<I - I / 2>()).mergesort(f), f);
      }
    };
    ...
    struct merger<I, J, typename std::enable_if<(I == 1 && J == 1)>::type> {
      template <typename F> constexpr static array<T, I + J> merge(const array<T, I>& a,
                                                                   const array<T, J>& b,
                                                                   const F& f) {
        return f(b[0], a[0]) ? array<T, I + J>{b[0], a[0]} : array<T, I + J>{a[0], b[0]};
      }
    };
    ...
    struct merger<I, J, typename std::enable_if<(I == 1 && J > 1)>::type> {
      template <typename F> constexpr static array<T, I + J> merge(const array<T, I>& a,
                                                                   const array<T, J>& b,
                                                                   const F& f) {
        return f(b[0], a[0]) ? merger<I, J - 1>::merge(a, b.tail(), f).push_front(b[0])
                             : b.push_front(a[0]);
      }
    };
    ...
    struct merger<I, J, typename std::enable_if<(I > 1 && J == 1)>::type> {
      template <typename F> constexpr static array<T, I + J> merge(const array<T, I>& a,
                                                                   const array<T, J>& b,
                                                                   const F& f) {
        return f(b[0], a[0]) ? a.push_front(b[0])
    ...
    template <typename P> struct pred_to_less_t {
  ...
  template <typename T, size_t N> constexpr auto make_array(const T (&a)[N]) -> array<T, N> {
    return array<T, N>(a);
  }

  // make an array from some values: decay them so that we can easily have
  // arrays of string literals
  template <typename E, typename... Es> constexpr auto make_array(E&& e, Es&&... es)
      -> array<std::decay_t<E>, 1 + sizeof...(Es)> {
    return array<std::decay_t<E>, 1 + sizeof...(Es)>(std::forward<std::decay_t<E>>(e),
  ...
  template <typename F, typename T, size_t N> constexpr auto transform(const array<T, N>& a, F&& f)
      -> decltype(a.map(f)) {
  ...
  namespace detail {
    ...
    constexpr array<T, sizeof...(Is)> reverse(const array<T, sizeof...(Is)>& a,
                                              std::integer_sequence<int, Is...>) {
      return array<T, sizeof...(Is)>{a.end()[-(Is + 1)]...};
    }
  }  // namespace detail

  template <typename T, size_t N> constexpr array<T, N> reverse(const array<T, N>& a) {
