  ...
  namespace detail {
    namespace md5 {
      ...
      constexpr md5sum sumadd(const md5sum& s1, const md5sum& s2) {
        return {{s1.h[0] + s2.h[0], s1.h[1] + s2.h[1], s1.h[2] + s2.h[2], s1.h[3] + s2.h[3]}};
      }
      ...
      constexpr uint32_t rotateL(uint32_t x, int n) { return (x << n) | (x >> (32 - n)); }
      constexpr uint32_t FF(uint32_t a, uint32_t b, uint32_t c, uint32_t d, uint32_t x, int s,
                            uint32_t ac) {
        return rotateL(a + F(b, c, d) + x + ac, s) + b;
      }
      constexpr uint32_t GG(uint32_t a, uint32_t b, uint32_t c, uint32_t d, uint32_t x, int s,
                            uint32_t ac) {
        return rotateL(a + G(b, c, d) + x + ac, s) + b;
      }
      constexpr uint32_t HH(uint32_t a, uint32_t b, uint32_t c, uint32_t d, uint32_t x, int s,
                            uint32_t ac) {
        return rotateL(a + H(b, c, d) + x + ac, s) + b;
      }
      constexpr uint32_t II(uint32_t a, uint32_t b, uint32_t c, uint32_t d, uint32_t x, int s,
                            uint32_t ac) {
        return rotateL(a + I(b, c, d) + x + ac, s) + b;
      }

      // initial md5sum
      constexpr md5sum init() { return {{0x67452301, 0xefcdab89, 0x98badcfe, 0x10325476}}; }
      ...
      constexpr uint32_t pad(int len) {
        return len == 0 ? 0x00000080
                        : len == 1 ? 0x00008000 : len == 2 ? 0x00800000 : len == 3 ? 0x80000000 : 0;
      }
      constexpr uint32_t origlenbytes(int origlen, int origlenpos) {
        return origlenpos == 0 ? static_cast<uint64_t>(origlen) * 8 & 0xffffffff
                               : origlenpos == -4 ? (static_cast<uint64_t>(origlen) >> 29) : 0;
      }
      constexpr schedule leftover(const char* buf, int len, int origlen, int origlenpos) {
        return {{word32le(buf, len) | pad(len) | origlenbytes(origlen, origlenpos),
                 word32le(len >= 4 ? buf + 4 : buf, len - 4) | pad(len - 4)
                     | origlenbytes(origlen, origlenpos - 4),
                 word32le(len >= 8 ? buf + 8 : buf, len - 8) | pad(len - 8)
                     | origlenbytes(origlen, origlenpos - 8),
                 word32le(len >= 12 ? buf + 12 : buf, len - 12) | pad(len - 12)
                     | origlenbytes(origlen, origlenpos - 12),
                 word32le(len >= 16 ? buf + 16 : buf, len - 16) | pad(len - 16)
                     | origlenbytes(origlen, origlenpos - 16),
                 word32le(len >= 20 ? buf + 20 : buf, len - 20) | pad(len - 20)
                     | origlenbytes(origlen, origlenpos - 20),
                 word32le(len >= 24 ? buf + 24 : buf, len - 24) | pad(len - 24)
                     | origlenbytes(origlen, origlenpos - 24),
                 word32le(len >= 28 ? buf + 28 : buf, len - 28) | pad(len - 28)
                     | origlenbytes(origlen, origlenpos - 28),
                 word32le(len >= 32 ? buf + 32 : buf, len - 32) | pad(len - 32)
                     | origlenbytes(origlen, origlenpos - 32),
                 word32le(len >= 36 ? buf + 36 : buf, len - 36) | pad(len - 36)
                     | origlenbytes(origlen, origlenpos - 36),
                 word32le(len >= 40 ? buf + 40 : buf, len - 40) | pad(len - 40)
                     | origlenbytes(origlen, origlenpos - 40),
                 word32le(len >= 44 ? buf + 44 : buf, len - 44) | pad(len - 44)
                     | origlenbytes(origlen, origlenpos - 44),
                 word32le(len >= 48 ? buf + 48 : buf, len - 48) | pad(len - 48)
                     | origlenbytes(origlen, origlenpos - 48),
                 word32le(len >= 52 ? buf + 52 : buf, len - 52) | pad(len - 52)
                     | origlenbytes(origlen, origlenpos - 52),
                 word32le(len >= 56 ? buf + 56 : buf, len - 56) | pad(len - 56)
                     | origlenbytes(origlen, origlenpos - 56),
                 word32le(len >= 60 ? buf + 60 : buf, len - 60) | pad(len - 60)
      ...
      constexpr md5sum round1step(const md5sum& sum, const uint32_t* block, int step) {
        return {{FF(sum.h[0], sum.h[1], sum.h[2], sum.h[3], block[step], r1shift[step & 3],
                    r1const[step]),
                 sum.h[1], sum.h[2], sum.h[3]}};
      }
      constexpr md5sum round2step(const md5sum& sum, const uint32_t* block, int step) {
        return {{GG(sum.h[0], sum.h[1], sum.h[2], sum.h[3], block[(1 + step * 5) % 16],
                    r2shift[step & 3], r2const[step]),
                 sum.h[1], sum.h[2], sum.h[3]}};
      }
      constexpr md5sum round3step(const md5sum& sum, const uint32_t* block, int step) {
        return {{HH(sum.h[0], sum.h[1], sum.h[2], sum.h[3], block[(5 + step * 3) % 16],
                    r3shift[step & 3], r3const[step]),
                 sum.h[1], sum.h[2], sum.h[3]}};
      }
      constexpr md5sum round4step(const md5sum& sum, const uint32_t* block, int step) {
        return {{II(sum.h[0], sum.h[1], sum.h[2], sum.h[3], block[(step * 7) % 16],
                    r4shift[step & 3], r4const[step]),
                 sum.h[1], sum.h[2], sum.h[3]}};
      }
      ...
      constexpr md5sum round1(const md5sum& sum, const uint32_t* msg, int n) {
        return n == 16 ? sum : rotateCL(round1(rotateCR(round1step(sum, msg, n)), msg, n + 1));
      }
      constexpr md5sum round2(const md5sum& sum, const uint32_t* msg, int n) {
        return n == 16 ? sum : rotateCL(round2(rotateCR(round2step(sum, msg, n)), msg, n + 1));
      }
      constexpr md5sum round3(const md5sum& sum, const uint32_t* msg, int n) {
        return n == 16 ? sum : rotateCL(round3(rotateCR(round3step(sum, msg, n)), msg, n + 1));
      }
      constexpr md5sum round4(const md5sum& sum, const uint32_t* msg, int n) {
        return n == 16 ? sum : rotateCL(round4(rotateCR(round4step(sum, msg, n)), msg, n + 1));
      }
      ...
      constexpr md5sum md5update(const md5sum& sum, const char* msg, int len, int origlen) {
        return len >= 64 ? md5update(md5transform(sum, init(msg)), msg + 64, len - 64, origlen)
                         : len >= 56 ? md5update(md5transform(sum, leftover(msg, len, origlen, 64)),
                                                 msg + len, -1, origlen)
      ...
      constexpr md5sum md5(const char* msg) { return md5withlen(msg, strlen(msg)); }
    }  // namespace md5
  }    // namespace detail
  constexpr md5sum md5(const char* s) { return detail::md5::md5(s); }
}  // namespace cx
