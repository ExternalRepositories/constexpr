...
namespace cx {
  namespace {
    namespace detail_c {
      ...
      template <int H, int L> struct flag1 { friend constexpr int adl_flag1(flag1<H, L>); };
      template <int H> struct flag2 { friend constexpr int adl_flag2(flag2<H>); };
      ...
      template <int H> struct r1 {
        template <int L, int = adl_flag1(flag1<H, L>{})>
        static constexpr int reader(int, flag1<H, L>) {
          return L;
        }
        template <int L>
        static constexpr int reader(float, flag1<H, L>, int R = reader(0, flag1<H, L - 1>{})) {
          return R;
        }
        static constexpr int reader(float, flag1<H, 0>) { return 0; }
      };

      // to read flag2, just overload/specialize the function
      template <int H, int = adl_flag2(flag2<H>{})> constexpr int reader(int, flag2<H>) {
        return H;
      }
      template <int H> constexpr int reader(float, flag2<H>, int R = reader(0, flag2<H - 1>{})) {
        return R;
      }
      constexpr int reader(float, flag2<0>) { return 0; }

      // write the low bits flag
      template <int H, int L> struct writelo {
        friend constexpr int adl_flag1(flag1<H, L>) { return L; }
        static constexpr int value = L;
      };
      // write the high bits flag (if it should be written)
      template <int H, bool B> struct writehi {
        friend constexpr int adl_flag2(flag2<H>) { return H; }
        static constexpr int value = H;
      };
      template <int H> struct writehi<H, false> { static constexpr int value = H; };
      ...
      template <int H, int L> struct writer {
        static constexpr int hi_value = writehi<H + 1, L == MAX>::value;
        static constexpr int lo_value = writelo<H, (L & BIT_MASK)>::value;
  ...
  template <int N = 1, int H = detail_c::reader(0, detail_c::flag2<detail_c::MAX>{}),
            int L = detail_c::r1<H>::reader(0, detail_c::flag1<H, detail_c::MAX>{})>
  inline constexpr int counter(int R = detail_c::writer<H, L + N>::value) {
    return R - 1;
  }
}  // namespace cx
