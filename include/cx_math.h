...
namespace cx {
  ...
  //----------------------------------------------------------------------------
  template <typename FloatingPoint> constexpr FloatingPoint abs(
      FloatingPoint x,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return x >= 0 ? x : x < 0 ? -x : throw "err::abs_runtime_error";
  }
  ...
  //----------------------------------------------------------------------------
  template <typename FloatingPoint> constexpr FloatingPoint fabs(
      FloatingPoint x,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return x >= 0 ? x : x < 0 ? -x : throw "err::fabs_runtime_error";
  }
  template <typename Integral> constexpr double fabs(
      Integral x, typename std::enable_if<std::is_integral<Integral>::value>::type* = nullptr) {
    return x >= 0 ? x : x < 0 ? -x : throw "err::fabs_runtime_error";
  }
  ...
  namespace detail {
    template <typename FloatingPoint> constexpr FloatingPoint ipow(
  ...
  namespace detail {
    template <typename T> constexpr T sqrt(T x, T guess) {
      return feq(guess, (guess + x / guess) / T{2}) ? guess : sqrt(x, (guess + x / guess) / T{2});
    }
  }  // namespace detail
  template <typename FloatingPoint> constexpr FloatingPoint sqrt(
      FloatingPoint x,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return x == 0 ? 0 : x > 0 ? detail::sqrt(x, x) : throw "err::sqrt_domain_error";
  }
  template <typename Integral> constexpr double sqrt(
      Integral x, typename std::enable_if<std::is_integral<Integral>::value>::type* = nullptr) {
  ...
  namespace detail {
    template <typename T> constexpr T cbrt(T x, T guess) {
      return feq(guess, (T{2} * guess + x / (guess * guess)) / T{3})
                 ? guess
                 : cbrt(x, (T{2} * guess + x / (guess * guess)) / T{3});
    }
  }  // namespace detail
  template <typename FloatingPoint> constexpr FloatingPoint cbrt(
      FloatingPoint x,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return detail::cbrt(x, FloatingPoint{1});
  }
  template <typename Integral> constexpr double cbrt(
      Integral x, typename std::enable_if<std::is_integral<Integral>::value>::type* = nullptr) {
  ...
  //----------------------------------------------------------------------------
  // hypot
  template <typename FloatingPoint> constexpr FloatingPoint hypot(
  ...
  template <typename A1, typename A2> struct promoted { using type = double; };

  template <typename A> struct promoted<long double, A> { using type = long double; };
  template <typename A> struct promoted<A, long double> { using type = long double; };
  template <> struct promoted<long double, long double> { using type = long double; };

  template <typename A1, typename A2> using promoted_t = typename promoted<A1, A2>::type;
  ...
  template <typename FloatingPoint> constexpr FloatingPoint exp(
      FloatingPoint x,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return detail::exp(x, FloatingPoint{1}, FloatingPoint{1}, 2, x);
  }
  template <typename Integral> constexpr double exp(
      Integral x, typename std::enable_if<std::is_integral<Integral>::value>::type* = nullptr) {
  ...
  namespace detail {
    template <typename T> constexpr T trig_series(T x, T sum, T n, int i, int s, T t) {
      return feq(sum, sum + t * s / n)
                 ? sum
                 : trig_series(x, sum + t * s / n, n * i * (i + 1), i + 2, -s, t * x * x);
    }
  }  // namespace detail
  template <typename FloatingPoint> constexpr FloatingPoint sin(
      FloatingPoint x,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return detail::trig_series(x, x, FloatingPoint{6}, 4, -1, x * x * x);
  }
  template <typename Integral> constexpr double sin(
      Integral x, typename std::enable_if<std::is_integral<Integral>::value>::type* = nullptr) {
    return sin<double>(x);
  }
  ...
  template <typename FloatingPoint> constexpr FloatingPoint cos(
      FloatingPoint x,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return detail::trig_series(x, FloatingPoint{1}, FloatingPoint{2}, 3, -1, x * x);
  }
  template <typename Integral> constexpr double cos(
      Integral x, typename std::enable_if<std::is_integral<Integral>::value>::type* = nullptr) {
    return detail::trig_series<double>(x, 1.0, 2.0, 3, -1,
  ...
  template <typename FloatingPoint> constexpr FloatingPoint tan(
      FloatingPoint x,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return cos(x) != 0 ? sin(x) / cos(x) : throw "err::tan_domain_error";
  }
  template <typename Integral> constexpr double tan(
      Integral x, typename std::enable_if<std::is_integral<Integral>::value>::type* = nullptr) {
    return cos(x) != 0.0 ? sin(x) / cos(x) : throw "err::tan_domain_error";
  }

  //----------------------------------------------------------------------------
  // arctan by Euler's series
  namespace detail {
    template <typename T> constexpr T atan_term(T x2, int k) {
      return (T{2} * static_cast<T>(k) * x2) / ((T{2} * static_cast<T>(k) + T{1}) * (T{1} + x2));
    }
    template <typename T> constexpr T atan_product(T x, int k) {
      return k == 1 ? atan_term(x * x, k) : atan_term(x * x, k) * atan_product(x, k - 1);
    }
    template <typename T> constexpr T atan_sum(T x, T sum, int n) {
      return sum + atan_product(x, n) == sum ? sum : atan_sum(x, sum + atan_product(x, n), n + 1);
    }
    constexpr long double pi() { return 3.1415926535897932385l; }
  }  // namespace detail
  template <typename FloatingPoint> constexpr FloatingPoint atan(
      FloatingPoint x,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return x / (FloatingPoint{1} + x * x) * detail::atan_sum(x, FloatingPoint{1}, 1);
  }
  template <typename Integral> constexpr double atan(
      Integral x, typename std::enable_if<std::is_integral<Integral>::value>::type* = nullptr) {
    return atan<double>(x);
  }

  template <typename FloatingPoint> constexpr FloatingPoint atan2(
      FloatingPoint x, FloatingPoint y,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return x > 0
               ? atan(y / x)
               : y >= 0 && x < 0
                     ? atan(y / x) + static_cast<FloatingPoint>(detail::pi())
                     : y < 0 && x < 0 ? atan(y / x) - static_cast<FloatingPoint>(detail::pi())
                                      : y > 0 && x == 0
                                            ? static_cast<FloatingPoint>(detail::pi() / 2.0l)
                                            : y < 0 && x == 0
                                                  ? -static_cast<FloatingPoint>(detail::pi() / 2.0l)
  ...
  namespace detail {
    template <typename T> constexpr T asin_series(T x, T sum, int n, T t) {
  ...
  template <typename FloatingPoint> constexpr FloatingPoint asin(
      FloatingPoint x,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return x == FloatingPoint{-1}
               ? detail::pi() / FloatingPoint{-2}
               : x == FloatingPoint{1}
                     ? detail::pi() / FloatingPoint{2}
  ...
  template <typename Integral> constexpr double asin(
      Integral x, typename std::enable_if<std::is_integral<Integral>::value>::type* = nullptr) {
    return asin<double>(x);
  }

  template <typename FloatingPoint> constexpr FloatingPoint acos(
      FloatingPoint x,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return x == FloatingPoint{-1}
               ? static_cast<FloatingPoint>(detail::pi())
               : x == FloatingPoint{1} ? 0
                                       : x > FloatingPoint{-1} && x < FloatingPoint{1}
                                             ? detail::pi() / FloatingPoint{2} - asin(x)
                                             : throw "err::acos_domain_error";
  }
  template <typename Integral> constexpr double acos(
      Integral x, typename std::enable_if<std::is_integral<Integral>::value>::type* = nullptr) {
  ...
  namespace detail {
    template <typename T> constexpr T floor2(T x, T guess, T inc) {
      return guess + inc <= x ? floor2(x, guess + inc, inc)
                              : inc <= T{1} ? guess : floor2(x, guess, inc / T{2});
    }
    template <typename T> constexpr T floor(T x, T guess, T inc) {
      return inc < T{8}
                 ? floor2(x, guess, inc)
                 : guess + inc <= x
                       ? floor(x, guess + inc, inc)
                       : guess + (inc / T{8}) * T{7} <= x
                             ? floor(x, guess + (inc / T{8}) * T{7}, inc / T{8})
                             : guess + (inc / T{8}) * T{6} <= x
                                   ? floor(x, guess + (inc / T{8}) * T{6}, inc / T{8})
                                   : guess + (inc / T{8}) * T{5} <= x
                                         ? floor(x, guess + (inc / T{8}) * T{5}, inc / T{8})
                                         : guess + (inc / T{8}) * T{4} <= x
                                               ? floor(x, guess + (inc / T{8}) * T{4}, inc / T{8})
                                               : guess + (inc / T{8}) * T{3} <= x
                                                     ? floor(x, guess + (inc / T{8}) * T{3},
                                                             inc / T{8})
                                                     : guess + (inc / T{8}) * T{2} <= x
                                                           ? floor(x, guess + (inc / T{8}) * T{2},
                                                                   inc / T{8})
                                                           : guess + inc / T{8} <= x
    ...
    template <typename T> constexpr T ceil2(T x, T guess, T dec) {
      return guess - dec >= x ? ceil2(x, guess - dec, dec)
                              : dec <= T{1} ? guess : ceil2(x, guess, dec / T{2});
    }
    template <typename T> constexpr T ceil(T x, T guess, T dec) {
      return dec < T{8}
                 ? ceil2(x, guess, dec)
                 : guess - dec >= x
                       ? ceil(x, guess - dec, dec)
                       : guess - (dec / T{8}) * T{7} >= x
                             ? ceil(x, guess - (dec / T{8}) * T{7}, dec / T{8})
                             : guess - (dec / T{8}) * T{6} >= x
                                   ? ceil(x, guess - (dec / T{8}) * T{6}, dec / T{8})
                                   : guess - (dec / T{8}) * T{5} >= x
                                         ? ceil(x, guess - (dec / T{8}) * T{5}, dec / T{8})
                                         : guess - (dec / T{8}) * T{4} >= x
                                               ? ceil(x, guess - (dec / T{8}) * T{4}, dec / T{8})
                                               : guess - (dec / T{8}) * T{3} >= x
                                                     ? ceil(x, guess - (dec / T{8}) * T{3},
                                                            dec / T{8})
                                                     : guess - (dec / T{8}) * T{2} >= x
                                                           ? ceil(x, guess - (dec / T{8}) * T{2},
                                                                  dec / T{8})
                                                           : guess - dec / T{8} >= x
  ...
  template <typename Integral> constexpr double ceil(
      Integral x, typename std::enable_if<std::is_integral<Integral>::value>::type* = nullptr);

  constexpr float floor(float x) {
    return x < 0 ? -ceil(-x)
                 : x >= 0 ? detail::floor(
                       x, 0.0f, detail::ipow(2.0f, std::numeric_limits<float>::max_exponent - 1))
                          : throw "err::floor_runtime_error";
  }
  constexpr double floor(double x) {
    return x < 0 ? -ceil(-x)
                 : x >= 0 ? detail::floor(
                       x, 0.0, detail::ipow(2.0, std::numeric_limits<double>::max_exponent - 1))
                          : throw "err::floor_runtime_error";
  }
  template <typename Integral> constexpr double floor(
      Integral x, typename std::enable_if<std::is_integral<Integral>::value>::type* = nullptr) {
    return x;
  }
  ...
  constexpr double ceil(Integral x,
...
// See above: long double floor/ceil only available for C++14 constexpr
  ...
  constexpr float trunc(float x) { return x >= 0 ? floor(x) : -floor(-x); }
  constexpr double trunc(double x) { return x >= 0 ? floor(x) : -floor(-x); }
#if __cplusplus == 201402L
  constexpr long double trunc(long double x) { return x >= 0 ? floor(x) : -floor(-x); }
#endif

  template <typename Integral> constexpr double trunc(
      Integral x, typename std::enable_if<std::is_integral<Integral>::value>::type* = nullptr) {
    return x;
  }

  constexpr float round(float x) { return x >= 0 ? floor(x + 0.5f) : ceil(x - 0.5f); }
  constexpr double round(double x) { return x >= 0 ? floor(x + 0.5) : ceil(x - 0.5); }
#if __cplusplus == 201402L
  constexpr long double round(long double x) { return x >= 0 ? floor(x + 0.5l) : ceil(x - 0.5l); }
#endif

  template <typename Integral> constexpr double round(
      Integral x, typename std::enable_if<std::is_integral<Integral>::value>::type* = nullptr) {
    return x;
  }
  ...
  constexpr float fmod(float x, float y) {
    return y != 0 ? x - trunc(x / y) * y : throw "err::fmod_domain_error";
  }
  ...
  template <typename A1, typename A2> struct cpp14_promoted { using type = double; };

#if __cplusplus == 201402L
  // Interestingly, this does not seem to produce a template instantiation
  // ambiguity with fmod_promoted<long double, long double>
  template <typename A> struct cpp14_promoted<long double, A> { using type = long double; };
  template <typename A> struct cpp14_promoted<A, long double> { using type = long double; };
#endif

  template <typename A1, typename A2> using cpp14_promoted_t =
      typename cpp14_promoted<A1, A2>::type;
  ...
  constexpr float remainder(float x, float y) {
    return y != 0 ? x - y * round(x / y) : throw "err::remainder_domain_error";
  }
  ...
  template <typename FloatingPoint> constexpr FloatingPoint fmax(
      FloatingPoint x, FloatingPoint y,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return x >= y ? x : x < y ? y : throw "err::fmax_runtime_error";
  }
  template <typename FloatingPoint> constexpr FloatingPoint fmin(
      FloatingPoint x, FloatingPoint y,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return y >= x ? x : y < x ? y : throw "err::fmin_runtime_error";
  }
  template <typename FloatingPoint> constexpr FloatingPoint fdim(
      FloatingPoint x, FloatingPoint y,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return x > y ? x - y : x <= y ? 0 : throw "err::fdim_runtime_error";
  }
  ...
  template <typename FloatingPoint> constexpr FloatingPoint log(
      FloatingPoint x,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return x < 0 ? throw "err::log_domain_error"
                 : x >= FloatingPoint{1024} ? detail::logLT(x) : detail::logGT(x);
  }
  template <typename Integral> constexpr double log(
      Integral x, typename std::enable_if<std::is_integral<Integral>::value>::type* = nullptr) {
  ...
  //----------------------------------------------------------------------------
  // other logarithms
  template <typename FloatingPoint> constexpr FloatingPoint log10(
      FloatingPoint x,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return log(x) / log(FloatingPoint{10});
  }
  template <typename Integral> constexpr double log10(
      Integral x, typename std::enable_if<std::is_integral<Integral>::value>::type* = nullptr) {
    return log10(static_cast<double>(x));
  }

  template <typename FloatingPoint> constexpr FloatingPoint log2(
      FloatingPoint x,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return log(x) / log(FloatingPoint{2});
  }
  template <typename Integral> constexpr double log2(
      Integral x, typename std::enable_if<std::is_integral<Integral>::value>::type* = nullptr) {
  ...
  //----------------------------------------------------------------------------
  // hyperbolic functions
  template <typename FloatingPoint> constexpr FloatingPoint sinh(
      FloatingPoint x,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return (exp(x) - exp(-x)) / FloatingPoint{2};
  }
  template <typename Integral> constexpr double sinh(
      Integral x, typename std::enable_if<std::is_integral<Integral>::value>::type* = nullptr) {
    return (exp<double>(x) - exp<double>(-x)) / 2.0;
  }

  template <typename FloatingPoint> constexpr FloatingPoint cosh(
      FloatingPoint x,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return (exp(x) + exp(-x)) / FloatingPoint{2};
  }
  template <typename Integral> constexpr double cosh(
      Integral x, typename std::enable_if<std::is_integral<Integral>::value>::type* = nullptr) {
    return (exp<double>(x) + exp<double>(-x)) / 2.0;
  }

  template <typename FloatingPoint> constexpr FloatingPoint tanh(
      FloatingPoint x,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return cosh(x) != 0 ? sinh(x) / cosh(x) : throw "err::tanh_domain_error";
  }
  template <typename Integral> constexpr double tanh(
      Integral x, typename std::enable_if<std::is_integral<Integral>::value>::type* = nullptr) {
    return cosh(x) != 0.0 ? sinh(x) / cosh(x) : throw "err::tanh_domain_error";
  }

  //----------------------------------------------------------------------------
  // inverse hyperbolic functions
  template <typename FloatingPoint> constexpr FloatingPoint asinh(
      FloatingPoint x,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return log(x + sqrt(x * x + FloatingPoint{1}));
  }
  template <typename Integral> constexpr double asinh(
      Integral x, typename std::enable_if<std::is_integral<Integral>::value>::type* = nullptr) {
    return asinh<double>(x);
  }

  template <typename FloatingPoint> constexpr FloatingPoint acosh(
      FloatingPoint x,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return x >= 1 ? log(x + sqrt(x * x - FloatingPoint{1})) : throw "err::acosh_domain_error";
  }
  template <typename Integral> constexpr double acosh(
      Integral x, typename std::enable_if<std::is_integral<Integral>::value>::type* = nullptr) {
    return acosh<double>(x);
  }

  template <typename FloatingPoint> constexpr FloatingPoint atanh(
      FloatingPoint x,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return x > -1 && x < 1 ? (FloatingPoint{1} / FloatingPoint{2})
                                 * log((FloatingPoint{1} + x) / (FloatingPoint{1} - x))
                           : throw "err::atanh_domain_error";
  }
  template <typename Integral> constexpr double atanh(
      Integral x, typename std::enable_if<std::is_integral<Integral>::value>::type* = nullptr) {
  ...
  template <typename FloatingPoint> constexpr FloatingPoint pow(
      FloatingPoint x, FloatingPoint y,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return exp(log(x) * y);
  }
  ...
  template <typename Integral> constexpr promoted_t<Integral, int> pow(
      Integral x, int y,
  ...
  namespace detail {
    constexpr long double two_over_root_pi() { return 1.128379167095512573896l; }

    template <typename T> constexpr T erf(T x, T sum, T n, int i, int s, T t) {
      return feq(sum, sum + (t * s / n) / (2 * i + 1))
                 ? sum
                 : erf(x, sum + (t * s / n) / (2 * i + 1), n * (i + 1), i + 1, -s, t * x * x);
    }
  }  // namespace detail
  template <typename FloatingPoint> constexpr FloatingPoint erf(
      FloatingPoint x,
      typename std::enable_if<std::is_floating_point<FloatingPoint>::value>::type* = nullptr) {
    return detail::erf(x, x, FloatingPoint{1}, 1, -1, x * x * x) * detail::two_over_root_pi();
  }
  template <typename Integral> constexpr double erf(
      Integral x, typename std::enable_if<std::is_integral<Integral>::value>::type* = nullptr) {
    return erf<double>(x);
  }
}  // namespace cx
