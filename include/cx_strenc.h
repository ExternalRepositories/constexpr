...
namespace cx {
  ...
  namespace detail {
    ...
    template <uint64_t S> constexpr char encrypt_at(const char* s, size_t idx) {
      return s[idx] ^ static_cast<char>(pcg::pcg32_output(pcg::pcg32_advance(S, idx + 1)) >> 24);
    }

    // store the string in a char_array for constexpr manipulation
    template <size_t N> struct char_array { char data[N]; };
  ...
  public:
    ...
    operator std::string() const { return detail::decrypt(S, m_enc.data, N - 1); }
