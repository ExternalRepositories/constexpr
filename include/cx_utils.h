...
namespace cx {
  ...
  constexpr int strless(const char* a, const char* b) { return strcmp(a, b) == -1; }
  ...
  constexpr uint32_t word32be(const char* s, int len) {
    return (len > 0 ? (static_cast<uint32_t>(s[0]) << 24) : 0)
  ...
  constexpr uint32_t word32be(const char* s) { return word32be(s, 4); }

  // swap endianness of various size integral types
  constexpr uint64_t endianswap(uint64_t x) {
    return ((x & 0xff) << 56) | (((x >> 8) & 0xff) << 48) | (((x >> 16) & 0xff) << 40)
           | (((x >> 24) & 0xff) << 32) | (((x >> 32) & 0xff) << 24) | (((x >> 40) & 0xff) << 16)
